    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.chapters.update", [$chapter->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $chapter->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('report') ? 'is-invalid' : '' }}" name="report_id" id="report_id" required>
                    @foreach($reports as $id => $entry)
                        <option value="{{ $id }}" {{ (old('report_id') ? old('report_id') : $chapter->report->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number') ? 'is-invalid' : '' }}" type="number" name="number" id="number" value="{{ old('number', $chapter->number) }}" step="1" required>
